...
drivers (`irq-aspeed-g3-vic`, plus the `ftgmac100` `FAST_MODE` RX fix and the
`clk-aspeed` AST2050 clock patch). "Not register-compatible" means the mainline
...
* - `gpio-aspeed`
  ...
  - [GPIO][reg-buses]
  - G3 binds on `ast2400-gpio`; pairs with `pinctrl-aspeed`
* - `pinctrl-aspeed`
  - pinctrl
...
**`ftgmac100`** drives the Faraday-derived 10/100 MAC (with internal MDIO) that
...
**`i2c-aspeed`** drives the AST2050 I2C/SMBus engine (14 buses); on G3 it binds
...
**`spi-aspeed-smc`** is the mainline Aspeed SPI/flash controller driver, but it
...
**`gpio-aspeed`** drives the SoC GPIO banks (base `0x1E780000`) and binds on
`aspeed,ast2400-gpio` for G3; it works together with **`pinctrl-aspeed`** for
pin muxing. The G3 has no dedicated pinctrl compatible — its pin multiplexing is
strap/`SCU`-driven — so `pinctrl-aspeed`'s `aspeed,ast2400-pinctrl` (g4) is used
where a pinctrl node is needed. See [GPIO][reg-buses].

**`clk-aspeed`** is the SCU clock driver (`aspeed,ast2400-scu` /
...
**`timer-fttmr010`** drives the Faraday FTTMR010 timer block used for the system
...
**`8250` / `8250_aspeed_vuart`** — the SoC UARTs are standard 16550s driven by
the generic `8250` core (DT `ns16550a`); the separate `8250_aspeed_vuart` driver
...
**`aspeed-pwm-tacho`** drives the SoC PWM & fan-tachometer controller at
...
**`rtc-aspeed`** drives the AST2400+ RTC (`aspeed,ast2400-rtc`). It is **not
...
**`peci-aspeed`** drives the PECI controller (`aspeed,ast2400-peci`); the G3
block is at `0x1E78B000` and is unused on both boards. See [PECI][reg-ctrl].

**`aspeed-vhub`** drives the USB 2.0 virtual-hub UDC at `0x1E6A0000`
(`aspeed,ast2400-usb-vhub`) — the gadget side of the vKVM HID path — and
**`aspeed-video`** drives the Video (capture/compression) Engine at `0x1E700000`
...
**`drm/ast` vs `drm/aspeed`** — these are easy to confuse. `drm/ast` is the
...
BMC. `drm/aspeed` (the `aspeed-gfx` driver, `aspeed,ast2400-gfx` /
...
**`pca953x`** (`gpio-pca953x`, `nxp,pca9555`) drives the five PCA9555 I2C GPIO
expanders on the C410X ([PCA9555][per-pca9555]). **`i2c-mux-pca954x`**
(`nxp,pca9548` / `nxp,pca9544`) drives the C410X I2C mux fan-out
([PCA9548 / PCA9544][per-pca954x]). **`ina2xx`** (`ti,ina219`) drives the
sixteen INA219 rail monitors ([INA219][per-ina219]). **`adt7462`**
(`adi,adt7462`, matched by the `adt7462` I2C client name) drives the two ADT7462
fan/temp controllers ([ADT7462][per-adt7462]). **`lm75`** (`ti,tmp75`) drives the
sixteen TMP75 board-temperature sensors ([TMP75 / LM75][per-tmp75]). **`w83795`**
...
[Interrupt controller (VIC)][reg-uvt]. The **`ftgmac100` `FAST_MODE` fix** and
the **`clk-aspeed` AST2050 patch** are patches to the mainline drivers above
