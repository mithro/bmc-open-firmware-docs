...
* - {doc}`MAC (ftgmac100) <../hardware/registers/network-mac-phy>`
  ...
  - `ftgmac100`
* - MDIO / MII
  - `ftgmac100` internal MDIO / `mdio-aspeed`
  - within MAC
  - port needed
  - within MAC model
* - PHY (RTL8201CP)
...
* - {doc}`SPI / SMC flash <../hardware/registers/buses-gpio>`
  - `spi-aspeed-smc` (legacy-SMC caveat)
  - `spi_flash` / SMC
  - port needed
  - `aspeed-smc`
...
* - {doc}`UART <../hardware/registers/uart-vic-timers>`
  - `8250` / `of_serial` (16550)
  - `ns16550`
  - Zephyr `uart_ns16550`
  - `serial-16550`
* - {doc}`VGA / Video Engine <../hardware/registers/pcie-vga-usb-bridges>`
  - `drm/aspeed` (display); video-engine via `aspeed-video` (V4L2)
  - —
  - out of scope
...
* - {doc}`W83795G <../hardware/peripherals/w83795g>`
  - KGPE-D16
  - `w83795` (hwmon; **host** SP5100 SMBus)
