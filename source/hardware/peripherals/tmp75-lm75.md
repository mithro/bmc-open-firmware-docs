...
resolution and a few configuration bits (detailed below). [TMP75 DS p.16] [LM75 DS p.16]
...
- **16× per-slot TMP75-class sensors** on bus `0xF4` (`&i2c4`), all at 7-bit `0x5C`,
  ...
  [IS_fl.bin.md:102-129] [dts:733-851] [ANALYSIS.md:499]
- **1× LM75 front-board sensor** on bus `0xF6` (`&i2c6`) at 7-bit `0x4F` (8-bit
  ...
  [IS_fl.bin.md:131-139] [dts:1104-1119] [ANALYSIS.md:501]
...
LM75-class variant rather than a stock TMP75.) [dts:739-743] [TMP75 DS p.11]
...
pointer is retained across reads. [TMP75 DS p.11,16] [LM75 DS p.15-16]
...
* - `0x03`
  ...
  - 12-bit / 9-bit
```
[TMP75 DS p.16,20] [LM75 DS p.16-17]
...
common layout is why one Linux driver serves both. [TMP75 DS p.20] [LM75 DS p.14]

## Temperature register (0x00) & conversion

Read-only, two bytes, MSB byte first, left-justified two's-complement; reads 0 °C
until the first conversion completes. [TMP75 DS p.17] [LM75 DS p.16]
...
* - 7
  - OS
  - One-shot: write 1 while shut down to start a single conversion; always reads 0. [TMP75 DS p.19]
  - Reserved — keep 0 (LM75 has no one-shot). [LM75 DS p.16]
* - 6
  - R1
  - Converter resolution MSB (see table). [TMP75 DS p.19]
  - Reserved — keep 0. [LM75 DS p.16]
* - 5
  - R0
  - Converter resolution LSB (see table). [TMP75 DS p.19]
  - Reserved — keep 0. [LM75 DS p.16]
* - 4
  - F1
  - Fault-queue count MSB (see table). [TMP75 DS p.18]
  - Fault-queue count MSB. [LM75 DS p.16]
* - 3
  - F0
  - Fault-queue count LSB (see table). [TMP75 DS p.18]
  - Fault-queue count LSB. [LM75 DS p.16]
* - 2
  - POL
  - ALERT polarity: 0 = active-low (default), 1 = active-high. [TMP75 DS p.18]
  - O.S. polarity: 0 = active-low, 1 = active-high. [LM75 DS p.16]
* - 1
  - TM
  - Thermostat mode: 0 = comparator, 1 = interrupt. [TMP75 DS p.17]
  - Cmp/Int: 0 = comparator, 1 = interrupt. [LM75 DS p.16]
* - 0
  - SD
  - Shutdown: 1 = shutdown (< 0.1 µA) after current conversion; 0 = continuous. [TMP75 DS p.18]
  - Shutdown: 1 = low-power shutdown; 0 = continuous. [LM75 DS p.16]
```
...
* - `11`
  ...
  - 220 ms
```
[TMP75 DS p.19] (LM75 is fixed 9-bit / 0.5 °C, ~100 ms typ. [LM75 DS p.5,12])
...
[TMP75 DS p.20] [LM75 DS p.5,13,17]
...
- **POL** inverts the output polarity (default active-low). The LM75 O.S. pin is
  ...
  responder (Alert command `0x19`), which the LM75 O.S. output cannot. [TMP75 DS p.18,11] [LM75 DS p.15]
...
state for 27 addresses. [TMP75 DS p.11] [LM75 DS p.13]
...
```
[TMP75 DS p.3-5,9-12,17-20] [LM75 DS p.1,3-5,12-17]
...
"national,lm75"` (front board). [dts:769,1116] [lm75.c] The driver's registers are
...
(`set_mask = 3<<5`, `clr_mask = 1<<7`). [lm75.c]
...
- The driver's `lm75b` type models the **NXP LM75B (11-bit)**, which is *not* the
  ...
  datasheet part, use `national,lm75` (9-bit), **not** `national,lm75b`. [lm75.c]
```
