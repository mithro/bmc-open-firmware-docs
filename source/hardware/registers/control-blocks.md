...
32.768 KHz crystal on the AST2050. [DS §8.1 p.84]
...
[aspeed-driver-quick-reference.md:4,107,114]
...
- Offsets not listed in a block's register map are **reserved** on the G3 and
  ...
  unpredictable result. [DS §9 p.97]
```
...
* - PWM & Fan Tacho
  - 0x1E786000
  - 28 — Tachometer interrupt (sensitive high level) [DS §10 p.99]
  - PWMCLK 24 MHz, TACHCLK 6 MHz [DS §8.1 p.84]
  - SCU04[9] reset PWM (default = held in reset); block clock gated by PTCR00[0] [DS p.205]
  - PWM1–4 = GPIOC2–5 via SCU74[8]–SCU74[11]; tach inputs shared with GPIOE / DVO [DS §7 p.80] [DS §28.2 p.290]
* - RTC
  - 0x1E781000
  - 22 second, 23 day, 24 hour, 25 minute, 26 alarm (all edge / both-edge) [DS §10 p.99]
  - CLK1M 1 MHz [DS §8.1 p.84]
  - No dedicated SCU reset bit; software reset via RTC14 = 0x99 [DS p.272]
  - Dedicated pins (no mux) [DS §24 p.270]
* - PECI
  - 0x1E78B000
  - 15 — PECI interrupt (sensitive high level) [DS §10 p.99]
  - PECICLK 2 MHz [DS §8.1 p.84]
  - SCU04[10] reset PECI (default = held in reset); block clock gated by PECI00[0] [DS p.205]
  - PECII = GPIOC0, PECIO = GPIOC1, both via SCU74[7] [DS §7 p.80]
* - VUART / PUART
  - 0x1E787000 / 0x1E788000
  - none dedicated — ARM side via LPC INT #8; host side via LPC SerIRQ (see notes) [DS §10 p.99]
  - LCLK 33 MHz (LPC bus clock — sets the fixed baud) [DS §8.1 p.84]
  - SCU04[5] reset LPC/BMC; UARTCLK gate SCU0C[15], LCLK gate SCU0C[8] [DS p.206,208]
  - Data path only (PUART uses UART1/UART2 pins) [DS §29 p.296]
```
...
VUART/PUART entry.) [DS §10 p.99] [DS §29 p.306,308]
...
pins revert to GPIO. [DS §28.1–28.2 p.290]
...
exist". [DS §28.1 p.290] [DS revision history, v0.92 (Mar 05 2008)]
...
* - 0x00
  ...
  - 0xXXXXX000
  - RW
  - Per-channel fan-tach enables [31:16], per-port PWM type select [15:12], per-port PWM enables [11:8], global PWM & fan-tach clock enable [0]. [DS p.291]
* - 0x04
  - PTCR04 Clock Control
  - X
  - RW
  - Type N & Type M PWM period [7:0] and two-stage clock dividers (H = power-of-two ÷1…÷32768; L = even ÷1…÷30). [DS p.291-292]
* - 0x08
  - PTCR08 Duty Control 0
  - X
  - RW
  - PWM B falling [31:24] / rising [23:16] point, PWM A falling [15:8] / rising [7:0] point (in units of PWM period). [DS p.292]
* - 0x0C
  - PTCR0C Duty Control 1
  - X
  - RW
  - PWM D falling [31:24] / rising [23:16] point, PWM C falling [15:8] / rising [7:0] point. [DS p.292]
* - 0x10
  - PTCR10 Type M Control 0
  - X
  - RW
  - Type M fan-tach period [31:16], smart-fan-tach enable [7], edge mode [5:4], clock divide [3:1], tach enable [0]. [DS p.292-293]
* - 0x14
  - PTCR14 Type M Control 1
  - X
  - RW
  - Type M fan-tach falling point [31:16] / rising point [15:0] of period. [DS p.293]
* - 0x18
  - PTCR18 Type N Control 0
  - X
  - RW
  - Type N fan-tach period [31:16], smart-fan-tach enable [7], edge mode [5:4], clock divide [3:1], tach enable [0]. [DS p.293]
* - 0x1C
  - PTCR1C Type N Control 1
  - X
  - RW
  - Type N fan-tach falling point [31:16] / rising point [15:0] of period. [DS p.293]
* - 0x20
  - PTCR20 Tach Source
  - X
  - RW
  - 2 bits per tach channel selecting its PWM source (00=A,01=B,10=C,11=D); channel *n* uses bits [2n+1:2n]. [DS p.294]
* - 0x24
  - *(reserved / not implemented)*
  - —
  - —
  - **Gap.** No register at this offset. [DS §28.1 p.290 register list]
* - 0x28
  - PTCR28 Trigger
  - X
  - RW
  - 0-to-1 trigger to start a fan-tach read, one bit per channel [15:0]; upper half [31:16] reserved. [DS p.294]
* - 0x2C
  - PTCR2C Result
  - X
  - RO
  - Full-measurement status [31], measured tach value [19:0]; [30:20] reserved. [DS p.294]
* - 0x30
  - PTCR30 Interrupt Control
  - X
  - RW
  - Per-channel fan-tach interrupt enable [15:0]; [31:16] reserved. [DS p.294-295]
* - 0x34
  - PTCR34 Interrupt Status
  - X
  - RW
  - Per-channel fan-tach interrupt pending [15:0] (drives VIC #28); [31:16] reserved. [DS p.295]
* - 0x38
  - PTCR38 Type M Limit
  - X
  - RW
  - Type M fan-tach over-speed limit [19:0]; [31:20] reserved. [DS p.295]
* - 0x3C
  - PTCR3C Type N Limit
  - X
  - RW
  - Type N fan-tach over-speed limit [19:0]; [31:20] reserved. [DS p.295]
```
...
* - 31:16
  ...
  - RW
  - One bit per channel; bit[16+n] enables tach #n. 0=disable, 1=enable. [DS p.291]
* - 15:12
  - PWM D/C/B/A type select
  - RW
  - Bit[15]=port D, [14]=C, [13]=B, [12]=A. 0=Type M, 1=Type N. [DS p.291]
* - 11:8
  - PWM D/C/B/A enable
  - RW
  - Bit[11]=port D, [10]=C, [9]=B, [8]=A. 0=disable, 1=enable. [DS p.291]
* - 7:1
  - Reserved
  - RW
  - Reserved. [DS p.291]
* - 0
  ...
  - RW
  - Global gate for the block. 0=disable, 1=enable. [DS p.291]
```
...
`0010`=÷4, `0011`=÷6 … `1111`=÷30). PWMCLK = 24 MHz ÷ (H × L). [DS p.291-292]
...
* - 31:24
  ...
  - RW
  - PWM period in Type-N PWM clocks. [DS p.291]
* - 23:20
  - Type N clock divide H [3:0]
  - RW
  - Power-of-two divider ÷1…÷32768. [DS p.291]
* - 19:16
  - Type N clock divide L [3:0]
  - RW
  - Even divider ÷1…÷30. [DS p.292]
* - 15:8
  - Type M PWM period [7:0]
  - RW
  - PWM period in Type-M PWM clocks. [DS p.292]
* - 7:4
  - Type M clock divide H [3:0]
  - RW
  - Power-of-two divider ÷1…÷32768. [DS p.292]
* - 3:0
...
* - 31:16
  ...
  - RW
  - Measurement window, in units of the type's PWM clock. [DS p.292]
* - 15:8
  - Reserved (0)
  - RO
  - Reserved. [DS p.292]
...
* - 5:4
  ...
  - RW
  - 00=falling edge, 01=rising edge, 10=both edges, 11=reserved. [DS p.293]
* - 3:1
  - Fan-tach clock divide
  - RW
  - 000=÷4, 001=÷16, 010=÷64, 011=÷256 … 111=÷65536. [DS p.293]
* - 0
  - Fan-tach enable
  - RW
  - 0=disable, 1=enable. [DS p.293]
```
...
* - 27:2
  ...
  - RW
  - Two bits per channel, same 00/01/10/11 = A/B/C/D encoding. [DS p.294]
* - 1:0
  - Tach #0 PWM source
  - RW
  - 00=A, 01=B, 10=C, 11=D. [DS p.294]
...
* - 31
  ...
  - RO
  - 0=partial measurement, 1=full measurement. [DS p.294]
* - 30:20
  - Reserved (0)
  - RO
  - Reserved. [DS p.294]
* - 19:0
  ...
  - RO
  - Raw edge-count for the last triggered channel. [DS p.294]
```
...
fan-tach clock divide from `PTCR10[3:1]` / `PTCR18[3:1]` (÷4…÷65536). [DS p.295]
...
which is register-compatible with this G3 block. [aspeed-mainline-drivers-analysis.md:99,105]
...
status (`1`=interrupt pending) and is the source of VIC line #28. [DS p.294-295]
...
* - 15:0
  ...
  - RW
  - Bit *n* = fan-tach #n. PTCR30: 0=disable/1=enable. PTCR34: 0=no interrupt/1=pending. [DS p.295]
```
...
is ≈ 50 ppm (~1 s per 12 h). [DS §24.1–24.2 p.270]

The block implements **6 registers** at offsets `0x00`–`0x14`; all higher
offsets in the 4 KiB window are reserved. [DS §24.1 p.270]
...
* - 0x00
  ...
  - X
  - RO
  - Live counters: DayCnt [31:17], HourCnt [16:12] (0–23), MinuCnt [11:6] (0–59), SecCnt [5:0] (0–59). [DS p.270-271]
* - 0x04
  - RTC04 Clock Alarm
  - X
  - RW
  - Hour alarm [16:12], minute alarm [11:6], second alarm [5:0]. Out-of-range value = alarm never fires. [DS p.271]
* - 0x08
  - RTC08 Reload Value
  - X
  - RW
  - Day [31:17], hour [16:12], minute [11:6], second [5:0] loaded into the counters on restart. [DS p.271-272]
* - 0x0C
  - RTC0C Control
  - X
  - RW/RO
  - Restart-busy status [5] (RO), day/hour/minute/second alarm enables [4:1], RTC enable [0]. [DS p.272]
* - 0x10
  - RTC10 Restart
  - X
  - WO
  - Write `0x5A` [7:0] to reload the counters from RTC08 (whether or not RTC is enabled). [DS p.272]
* - 0x14
  - RTC14 Reset
  - X
  - RW
  - Write `0x99` [7:0] to reset the RTC immediately; write `0x00` to clear reset. [DS p.272]
```
...
* - 31:17
  - DayCnt
  - RO
  - Day counter; increments once per day, holds value when RTC disabled. [DS p.270]
* - 16:12
  - HourCnt
  - RO
  - Hour counter 0–23; wraps to 0 after 23. [DS p.270-271]
* - 11:6
  - MinuCnt
  - RO
  - Minute counter 0–59; wraps to 0 after 59. [DS p.271]
* - 5:0
  - SecCnt
  - RO
  - Second counter 0–59; wraps to 0 after 59. [DS p.271]
```
...
* - 5
  - Restart status
  - RO
  - 1 = RTC is currently reloading the reload value into the counters; 0 = not in restart period. Self-clears. [DS p.272]
* - 4
  - Day alarm enable
  - RW
  - 0=disable, 1=enable → VIC #23 (day). [DS p.272]
* - 3
  - Hour alarm enable
  - RW
  - 0=disable, 1=enable → VIC #24 (hour). [DS p.272]
* - 2
  - Minute alarm enable
  - RW
  - 0=disable, 1=enable → VIC #25 (minute). [DS p.272]
* - 1
  - Second alarm enable
  - RW
  - 0=disable, 1=enable → VIC #22 (second). [DS p.272]
* - 0
  - RTC enable
  - RW
  - 0=disable (default), 1=enable counting. [DS p.272]
```
...
in whether/when they poll `RTC0C[5]`. [DS §24.4 p.273-274]
```
...
mainline driver cannot bind to this block unmodified. [aspeed-mainline-drivers-analysis.md:150] [aspeed-driver-quick-reference.md:83]
...
PECII/PECIO pins are shared with GPIOC0/GPIOC1. [DS §32.1–32.2 p.357]
...
* - 0x00
  ...
  - 0x000XXX00
  - RW
  - Read sample point [19:16], read mode [13:12], PECI clock divider [11:8], I/O polarity [7:6], bus contention en [5], PECI enable [4], AW-FCS auto-gen [1], PECI clock enable [0]. [DS p.357-359]
* - 0x04
  - PECI04 Timing Negotiation
  - X
  - RW
  - Message timing negotiation [15:8], address timing negotiation [7:0] (unit = 4× PECI clock period). [DS p.359]
* - 0x08
  - PECI08 Command
  - 0
  - RW/RO
  - PECI pin monitor [31] (RO), controller FSM state [27:24] (RO), fire command [0]. [DS p.359]
* - 0x0C
  - PECI0C Read/Write Length
  - X
  - RW
  - AW-FCS-cycle enable [31], read length bytes [23:16], write length bytes [15:8], target address [7:0]. [DS p.359-360]
* - 0x10
  - PECI10 Expected FCS Data
  - X
  - RW/RO
  - Programmed AW FCS [31:24] (RW), expected read/AW/write FCS [23:0] (RO, debug). [DS p.360]
* - 0x14
  - PECI14 Captured FCS Data
  - X
  - RO
  - Captured read-command FCS [23:16], captured write-command FCS [7:0]. [DS p.360]
* - 0x18
  - PECI18 Interrupt (enable)
  - 0
  - RW
  - Timing-negotiation result select [31:30], enables: bus contention [3], write-FCS-bad [2], write-FCS-abort [1], command-done [0]. [DS p.360-361]
* - 0x1C
  - PECI1C Interrupt Status
  - 0xXXXX0000
  - R/W1C
  - Timing negotiation result [31:16] (RO), W1C status: contention [3], FCS-bad [2], FCS-abort [1], done [0] (source of VIC #15). [DS p.361]
* - 0x20
  - PECI20 Write Data #0
  - X
  - RW
  - Write data bits [31:0]. [DS p.361]
* - 0x24
  - PECI24 Write Data #1
  - X
  - RW
  - Write data bits [63:32]. [DS p.361]
* - 0x28
  - PECI28 Write Data #2
  - X
  - RW
  - Write data bits [95:64]. [DS p.361]
* - 0x2C
  - PECI2C Write Data #3
  - X
  - RW
  - Write data bits [127:96]. [DS p.361]
* - 0x30
  - PECI30 Read Data #0
  - X
  - RO
  - Read data bits [31:0]. [DS p.362]
* - 0x34
  - PECI34 Read Data #1
  - X
  - RO
  - Read data bits [63:32]. [DS p.362]
* - 0x38
  - PECI38 Read Data #2
  - X
  - RO
  - Read data bits [95:64]. [DS p.362]
* - 0x3C
  - PECI3C Read Data #3
  - X
  - RO
  - Read data bits [127:96]. [DS p.362]
```
...
* - 19:16
  - Read sampling point
  - RW
  - `0000`=0/16 … `1111`=15/16 of a bit-time; Point-Sampling mode only (middle is best). [DS p.358]
* - 15:14
  - Reserved
  - RW
  - Reserved. [DS p.358]
* - 13:12
  - Read mode
  - RW
  - 00=Point Sampling, 01=Pulse-Width Counting, 10=Debugging (ping only), 11=invalid. [DS p.358]
* - 11:8
  - PECI clock divider
  - RW
  - `0000`=÷1, `0001`=÷2, `0010`=÷4, `0011`=÷8 … `0111`=÷128; others invalid. Source = 24 MHz. [DS p.358]
* - 7
  - Inverse PECI output polarity
  - RW
  - 0=normal, 1=inverse. [DS p.358]
* - 6
...
* - 1
  ...
  - RW
  - 1=hardware generates Assured-Write FCS; 0=use software value in PECI10[31:24]. [DS p.358]
* - 0
  - Enable PECI clock
  - RW
  - 0=stop 24 MHz source (power save), 1=enable. [DS p.359]
```
...
* - 31
  - PECI pin monitor
  - RO
  - Reads back the live PECI wire level. [DS p.359]
* - 30:28
  - Reserved (0)
  - RO
  - Reserved. [DS p.359]
* - 27:24
  - Controller state
  - RO
  - FSM: 0000=idle, 0001=fire, 0010/0011=addr timing negotiation, 0100=address, 0101=msg timing negotiation, 0110=W/R length, 0111=write data, 1001=write FCS, 1010=read data, 1011=read FCS, 1100=stop; others reserved. [DS p.359]
* - 23:1
  - Reserved (0)
  - RO
  - Reserved. [DS p.359]
* - 0
  - Fire a PECI command
  - RW
  - 0=no-op, 1=fire the command configured in PECI0C/PECI20…. [DS p.359]
```
...
* - 31
  - Enable AW-FCS cycle
  - RW
  - Write commands only: 1=append an AW-FCS cycle. [DS p.359]
* - 30:24
  - Reserved (0)
  - RO
  - Reserved. [DS p.359]
* - 23:16
  ...
  - RW
  - Bytes to read into PECI30–PECI3C (max 16 despite 8-bit field). [DS p.360]
* - 15:8
  - Write data length (bytes)
  - RW
  - Bytes to write from PECI20–PECI2C. [DS p.360]
* - 7:0
  - Target address
  - RW
  - 8-bit PECI client address for the fired command. [DS p.360]
```
...
clear** status bits (the OR of which is VIC #15). [DS p.360-361]
...
* - 31:30
  ...
  - RW
  - 00=1st addr-negotiation bit, 01=2nd addr-negotiation bit, 10=message negotiation, 11=reserved. [DS p.360]
* - 29:4
  - Reserved (0)
  - RO
  - Reserved. [DS p.360]
* - 3
  ...
  - RW
  - 0=disable, 1=enable. [DS p.360]
* - 2
  - Write-FCS-bad interrupt enable
  - RW
  - 0=disable, 1=enable. [DS p.361]
* - 1
...
* - 31:16
  ...
  - RO
  - Captured negotiation result selected by PECI18[31:30]. [DS p.361]
* - 15:4
  - Reserved (0)
  - RO
  - Reserved. [DS p.361]
* - 3
  ...
  - W1C
  - 1=pending; write 1 to clear. [DS p.361]
* - 2
...
not list an AST2050 compatible. [aspeed-mainline-drivers-analysis.md:38]
...
replacement). Each has a 16×8 transmit and receive FIFO. [DS §29.1–29.2 p.296]
...
16550 software compatibility and do not change the actual rate. [DS p.297-298,301]
...
control registers (there is no host-visible register there). [DS §29.3 p.296-297]
...
* - 0x00
  ...
  - X
  - RW
  - Receive-buffer / transmit-holding; DLL divisor-low when DLAB=1 (cosmetic — real baud = LCLK). Both sides. [DS p.297-298]
* - 0x04
  - IER (DLAB=0) / DLH (DLAB=1)
  - 0
  - RW
  - Interrupt enables (RX-data, THRE, RX-line-status, modem-status); host bit[7] = FIFO-½-full THRE mode (if VUART34[6]=1). DLH when DLAB=1. [DS p.298-299]
* - 0x08
  - IIR (R) / FCR (W)
  - 0xC1 / 0x01
  - RW
  - Interrupt-identity read / FIFO-control write (trigger level [7:6], TX-reset [2], RX-reset [1], FIFO-enable [0]). Both sides. [DS p.299-301]
* - 0x0C
  - LCR — Line Control
  - 0x03
  - RW
  - DLAB [7], break control [6], word length [1:0]; [5:2] reserved. Both sides. [DS p.301-302]
* - 0x10
  - MCR — Modem Control
  - 0
  - RW
  - Loopback [4], Out2 [3], Out1 [2], nRTS [1], nDTR [0]; **host adds** TX-FIFO-full status at bit[7] (RO). [DS p.303]
* - 0x14
  - LSR — Line Status
  - 0x60
  - RO
  - TX empty [6], THRE [5], break [4], overrun [1], data-ready [0]; [3:2] reserved. Both sides. [DS p.304]
* - 0x18
  - MSR — Modem Status
  - X
  - RO
  - nDSR/nDCD/nRI/nCTS complements [7:4] and delta bits DDCD [3], TERI [2], DDSR [1], DCTS [0]. Both sides. [DS p.304-305]
* - 0x1C
  - SCR — Scratch
  - X
  - RW
  - 8-bit scratch [7:0], no defined function. Both sides. [DS p.305]
* - 0x20
  - VUART20 General Control A (Slave)
  - 0b00x0xx00
  - RW/RO
  - Enable VUART [0], SIRQ polarity [1], slave timeout width [3:2], host-loopback status [4] (RO), disable Host-Tx-discard [5], host RX-FIFO-trigger status [7:6] (RO). ARM-only. [DS p.305-306]
* - 0x24
  - VUART24 General Control B (Slave)
  - 0bxxxx xx11
  - RW/RO
  - Host bits-per-char [1:0] (RO), host-side timeout [3:2], SIRQ number [7:4]. ARM-only. [DS p.306]
* - 0x28
  - VUART28 Address L (Slave)
  - X
  - RW
  - LPC base address low byte [7:0] for host access to VUART. ARM-only. [DS p.306]
* - 0x2C
  - VUART2C Address H (Slave)
  - X
  - RW
  - LPC base address high byte [15:8]. ARM-only. [DS p.306]
* - 0x30
  - VUART30 General Control E (Slave)
  - 0b0000 1110
  - RO
  - Slave TX-FIFO-full [7], THR read pointer [6:4], complement of host IIR status [3:0]. ARM-only. [DS p.307]
* - 0x34
  - VUART34 General Control F (Slave)
  - X
  - RW
  - Slave/host FIFO-½-full THRE mode [7]/[6], force-THRE [5], disable char-timeout slave [1]/host [0]; [4:2] reserved. ARM-only. [DS p.307]
* - 0x38
  - VUART38 General Control G (Slave)
  - X
  - RO
  - Slave-side THR read-back data [7:0]. ARM-only. [DS p.307]
* - 0x3C
  - VUART3C General Control H (Slave)
  - X
  - RO
  - Read-back of slave RX-FIFO interrupt trigger level [7:6]; [5:0] reserved. ARM-only. [DS p.307]
```
...
* - 3:1
  - Interrupt decode
  - RO
  - 000=modem-status, 001=THR empty, 010=RX-data available, 011=RX line status, 110=char timeout. [DS p.299-300]
* - 0
  - Interrupt pending (active-low)
  - RO
  - 0=an interrupt is pending, 1=none pending. [DS p.299]
```

Priority (highest→lowest): RX line status (011) → RX data available (010) →
char timeout (110) → THRE (001) → modem status (000). [DS p.299-300]
...
* - 7
  ...
  - RO
  - 1=transmit FIFO full. On the slave set this bit is reserved. [DS p.303]
* - 6:5
  - Reserved (0)
  - RO
  - Reserved. [DS p.303]
* - 4
  - Loopback
  - RW
  - 1=loopback (nDTR→nDSR, nRTS→nCTS, Out1→nRI, Out2→nDCD). [DS p.303]
* - 3
  - Out2
  - RW
  - In loopback drives nDCD. [DS p.303]
* - 2
  - Out1
  - RW
  - In loopback drives nRI. [DS p.303]
* - 1
  - nRTS control
  - RW
  - 0→nRTS=1, 1→nRTS=0. [DS p.303]
* - 0
  - nDTR control
  - RW
  - 0→nDTR=1, 1→nDTR=0. [DS p.303]
```
...
* - 7:6
  ...
  - RO
  - Read-back of the host-side receiver FIFO trigger level. [DS p.305]
* - 5
  - Disable Host-Tx-discard mode
  - RW
  - 0=enable (slave auto-discards when host TX FIFO not empty), 1=disable. [DS p.305]
* - 4
  - Host loopback status
  - RO
  - Read-back of host-side loopback mode. [DS p.305]
* - 3:2
  - Slave-side timeout width
  - RW
  - 00/01/10/11 select 1/64…1/512 s (or scaled values per PUART34[3] & SCU2C[13]). [DS p.305-306]
* - 1
  - SerIRQ polarity
  - RW
  - Selects asserted/high-Z levels of the host SerIRQ output when the interrupt is set/cleared. [DS p.306]
* - 0
  - Enable virtual UART
  - RW
  - 0=disable, 1=enable the VUART. [DS p.306]
```
...
`PUART34[7]`), reached by the host over LPC. [DS §29.4 p.308]
...
* - 0x20
  ...
  - 0b00x0xx00
  - RW/RO
  - Host RX-FIFO trigger status [7:6] (RO), host loopback status [4] (RO), SIRQ polarity [1], enable pass-through UART [0]; [5],[3:2] reserved. [DS p.308]
* - 0x24
  - PUART24 General Control B
  - 0bxxxx xx11
  - RW/RO
  - SIRQ number [7:4], host bits-per-char [1:0] (RO); [3:2] reserved. [DS p.308]
* - 0x28
  - PUART28 Address L
  - X
  - RW
  - LPC base address low byte [7:0] for host access to the pass-through UART. [DS p.309]
* - 0x2C
  - PUART2C Address H
  - X
  - RW
  - LPC base address high byte [15:8]. [DS p.309]
* - 0x30
  - PUART30 General Control E
  - 0
  - RO
  - Complement of host LCR[7:2] in [5:0], host RX-data-int enable complement [6], host THRE-int enable complement [7]. [DS p.309]
* - 0x34
  - PUART34 General Control F
  - X
  - RW
  - Pass-through target select [7] (0=UART1, 1=UART2), VUART timeout-width control slave [3]/host [2]; [6:4],[1:0] reserved. [DS p.309-310]
* - 0x38
...
* - 0x3C
  ...
  - 0x00
  - RO
  - Complement of host modem-status-int enable [7], complement of host RX-line-status-int enable [6]; [5:0] reserved. [DS p.310]
```
...
* - 7
  ...
  - RW
  - 0=pass-through UART1, 1=pass-through UART2. [DS p.309]
* - 6:4
  - Reserved
  - RW
  - Reserved. [DS p.310]
* - 3
  ...
  - RW
  - Selects the slave-side timeout scaling used by VUART20[3:2]. [DS p.310]
* - 2
  - VUART timeout width control (host)
  - RW
  - Selects the host-side timeout scaling used by VUART24[3:2]. [DS p.310]
* - 1:0
  - Reserved
  - RW
  - Reserved. [DS p.310]
...
- Mainline `drivers/tty/serial/8250/8250_aspeed_vuart.c`
  ...
  [aspeed-mainline-drivers-analysis.md:135] [aspeed-driver-quick-reference.md:96]
```
