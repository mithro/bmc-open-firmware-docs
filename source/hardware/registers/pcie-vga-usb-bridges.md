...
frame buffer. `[DS §33.1 p.363]`, `[DS §34.1 p.369]`
...
`culvert p2a vga` drives to reach the BMC's AHB from the host. `[DS §33 p.366]`,
`[DS §36.2 p.400]`, `[CULVERT-G3]`
```
...
its legacy I/O addresses (also memory-mapped for advanced OSes). `[DS §34 p.369]`
...
driver on top of DRAM already initialised by the ARM. `[DS §34.1 p.369]`
...
`[DS §18 p.218]`. The absolute ARM addresses follow the *DRAM size × VGA size*
map in `[DS §9 p.98]`. For the KGPE-D16 (64 MB DRAM, `SCU70[3:2]=00` → 8 MB VGA)
...
`[DS §18 p.209, p.213-214, p.211, p.215-216]`
...
up to 1920×1200×32bpp@60. Register highlights: `[DS §20 p.232-235]`
...
engine (bypasses AHB via M-Bus), 32-stage descriptor mode. `[DS §15.1-15.2 p.154]`

```{admonition} Base address 0x1E6A0000
:class: note

`Base of USB Hub = 0x1E6A0000`; physical = base + offset. `[DS §15.3 p.155]`
```
...
`[DS §18 p.209]` (SCU0C[14] note), `[DS §15.3.2 p.156]`. (USB1.1 clock is gated
...
`Base of AHBC = 0x1E600000`; 4 registers. `[DS §12.3 p.114]`, `[hwreg.h]`
...
debugging. `[DS §36.1 p.400]`. `SCU2C[8]` **Disable PCI-slave-to-AHB bus bridge**
is the coarse master gate (`0`=enabled, `1`=disabled). `[DS §18 p.214]`
...
dead-firmware board). `[TIMER-RCA]`, `[CULVERT-G3]`
```
...
Host-Interface-Control registers. `[DS §30 p.319-321]`
...
`aspeed,ast2050-ilpc-ahb-bridge` ops read exactly this bit. `[CULVERT-G3]`
```
...
when the ARM is disabled (see also `HICR5[19:16]`/`HICR5[13:12]`). `[DS §30 p.322]`,
`[DS §7 p.7]`
...
`[DS §18 p.205-220]`. `SCU3C[1]` (watchdog-reset flag) is the clean post-reset
"did the WDT fire?" witness used by the P2A boot sequence. `[P2A-BOOT]`
...
- **Uncertainty — RTC INT polarity:** Table 36 lists RTC INT#22–26 as "edge
  ...
  both-edge RTC handling is datasheet-derived, not HW-proven. `[g3-vic patch]`
