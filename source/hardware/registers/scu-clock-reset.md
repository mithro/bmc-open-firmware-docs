...
- The SCU input reference is **always a 24 MHz crystal** on the AST2050
  (there is no 25/48 MHz crystal option as on the AST2400). [DS §8.1 p.84]
  [pin R22 CLKIN, DS p.47] [RAPTOR-PORTING-GUIDE.md:40,232]
```
...
to re-lock. [DS §18.1 p.204] [DS §9 p.97] [hwreg.h:77]
...
* - 0x00
  ...
  - 0x00000000
  - RW
  - Write `0x1688A8A8` to unlock; any other value locks. Reads back `1` when unlocked, `0` when locked. [DS p.204] [hwreg.h:79]
* - 0x04
  - SCU04 System Reset Control
  - 0x000FFE5C
  - RW
  - Per-module asynchronous reset bits (SDRAM, AHB bridges, I2C, HAC, LPC, video, MAC1/2, PWM, PECI, USB2.0, MDMA, MIC, PCI host). Default holds most modules in reset. [DS p.205] [hwreg.h:80]
* - 0x08
  - SCU08 Clock Selection
  - 0xE3F00070
  - RW
  - LHCLK/PCLK/BHCLK dividers, RTC clock source, video-port-A delay, CPU throttle, ECLK & MCLK source selects. [DS p.207]
* - 0x0C
  - SCU0C Clock Stop Control
  - 0x000C3E8B
  - RW
  - Per-clock gate (stop) bits: ECLK, GCLK, MCLK, V1CLK, BCLK, DCLK, REFCLK, UCLK, LCLK, YCLK, USB2.0, UARTCLK, BHCLK. [DS p.209]
* - 0x10
  - SCU10 Frequency Counter Control
  - 0x00000000
  - RW/RO
  - Ring-oscillator / PLL frequency-measurement control and pass/finish status. [DS p.210]
* - 0x14
  - SCU14 Frequency Counter Measurement
  - 0x00000000
  - RO
  - 14-bit measured counter value; `freq = (24MHz/512)*(value+1)`. [DS p.211]
* - 0x18
  - SCU18 Interrupt Control & Status
  - 0x00000000
  - RW/W1C
  - Enable + status for VGA-cursor-change and VGA-scratch-change interrupts (status bits are W1C). [DS p.211]
* - 0x1C
  - SCU1C 32.768 KHz Error Correction
  - 0x0000001B
  - RW
  - Fine-tunes the RTC 32.768 KHz source: `RTCclk = 12MHz*128/(46848+corr)`. [DS p.211] [hwreg.h:86]
* - 0x20
  - SCU20 M-PLL Parameter
  - 0x00004291
  - RW
  - Memory-clock PLL. Numerator/denominator/output-divider/post-divider, bypass, power-down. Default ≈133 MHz. [DS p.211] [hwreg.h:87]
* - 0x24
  - SCU24 H-PLL Parameter
  - 0x00004291
  - RW
  - CPU/AHB PLL. Same field layout as M-PLL, plus strap-vs-register select (bit 18). Default from straps (100/133/166/200 MHz). [DS p.212] [hwreg.h:88]
* - 0x28
  - SCU28 Frequency Counter Comparison Range
  - 0x00000000
  - RW
  - Upper limit [29:16] and lower limit [13:0] for the SCU10 pass/fail compare. [DS p.213] [hwreg.h:89]
* - 0x2C
  - SCU2C Misc. Control
  - 0x00000000
  - RW
  - UART1/2 link & mux, UART div13 reference-clock enable (bit 12), YCLK invert, D1-PLL disable, video DAC, SMC output buffers, PCI-slave bridge disable. [DS p.213]
* - 0x30
  - SCU30 PCI Config Setting #1
  - 0x20001A03
  - RW
  - PCI Device ID [31:16] / Vendor ID [15:0] presented to the host. [DS p.214]
* - 0x34
  - SCU34 PCI Config Setting #2
  - 0x20001A03
  - RW
  - PCI Sub-System ID [31:16] / Sub-Vendor ID [15:0]. [DS p.214]
* - 0x38
  - SCU38 PCI Config Setting #3
  - 0x03000000
  - RW
  - PCI Class Code [31:8] / Revision ID [7:0]. [DS p.214]
* - 0x3C
  - SCU3C System Reset Control (reset flags)
  - 0x00000001
  - RW
  - EXTRST# (GPIOB7) enable, and the power-on / watchdog / external reset source flags. [DS p.214]
* - 0x40
  - SCU40 SOC Scratch #1 (VGA handshake, bits 31:0)
  - 0x00000000
  - RW
  - ARM→host scratch. Bits used by firmware: Linux-boot key [31:24]=0x5A, DRAM-init-select [7], DRAM-init-ready [6], MAC PHY mode [15:12]. [DS p.215] [hwreg.h:91]
* - 0x44
  - SCU44 SOC Scratch #2 (scratch bits 63:32)
  - 0x00000000
  - RW
  - ARM→host scratch, upper 32 bits; firmware stores "last service IRQ number". [DS p.216] [hwreg.h:92]
* - 0x48
  - (reserved on G3)
  - —
  - —
  - Not defined in the A3 datasheet. On the AST2400 this offset is the MAC clock-delay register (`AST_SCU_MAC_CLK`); treat as reserved on the AST2050. [DS §18.1 p.204] [RAPTOR_ENGINEERING_AST2050_ANALYSIS.md:320]
* - 0x4C
  - (reserved on G3)
  - —
  - —
...
* - 0x50–0x6C
  ...
  - 0x00000000
  - RO
  - Host→ARM scratch, 256 bits total (8×32). Read-only to the ARM; written by the host CPU. [DS p.216]
* - 0x70
  - SCU70 Hardware Trapping (strap)
  - strap-defined (Init listed 0)
  - RW/RO
  - Latched hardware straps: boot source, H-PLL freq, CPU:AHB ratio, boot speed, MAC mode, VGA memory size, PLL bypass, LPC reset pin, test mode. [DS p.217]
* - 0x74
  - SCU74 Multi-function Pin Control #1
  - 0x40048000
  - RW
  - Pin-mux enables: I2C#5–7, PWM1–4, PECI, MAC MDIO/PHYLINK, VGA/DDC, UART2 full pins, HCLK output, NOR-flash pins, PCI REQ/GNT. [DS p.219]
* - 0x78
  - SCU78 Multi-function Pin Control #2
  - 0x00000000
  - RW
  - Watchdog-reset event output enable (bit 3), video-port-A modes, PCI INTA# disable. [DS p.219]
* - 0x7C
  - SCU7C Silicon Revision ID
  - 0x00000202
  - RO
  - Chip-bonding option [9:8] + silicon revision ID [7:0]. AST2050/AST1100-A2 and -A3 both read `0x00000202`. [DS p.220] [hwreg.h:94]
```
...
status value, not the key. The initial state is **locked**. [DS p.204]
...
* - 31:0
  - Protection Key
  - RW
  - Write `0x1688A8A8` = unlock. Write any other value = lock. Read-back: `0x00000001` when unlocked, `0x00000000` when locked. [DS p.204]
```
...
[platform.S:240-247] [platform.S:334-336] [platform.S:590-592]
...
* - 31:29
  ...
  - RW
  - `000`=H-PLL/2 … `111`=H-PLL/16 (steps of 2). [DS p.207]
* - 28
  - LHCLK generate/output enable
  - RW
  - 0=LHCLK from external LCLK pin; 1=generated internally. [DS p.207]
* - 27:26
  - Reserved
  - RW
  - Don't use. [DS p.207]
* - 25:23
  - APB PCLK divider
  - RW
  - `000`=H-PLL/2 … `111`=H-PLL/16. PCLK frequency has LPC-related limits (see clock tree). [DS p.207]
* - 22:20
  - PCI host BHCLK divider
  - RW
  - `000`=H-PLL/2 … `111`=H-PLL/16. [DS p.207]
* - 19
  - BHCLK generate/output enable
  - RW
  - 0=from external BCLK pin; 1=generated internally. [DS p.207]
* - 18:17
  - Reserved
  - RW
  - Don't use. [DS p.207]
* - 16
  ...
  - RW
  - 0=32.768 KHz source; 1=24 MHz source. [DS p.208]
* - 15:11
  - Reserved
  - RW
  - Keep 0. [DS p.208]
* - 10:8
  ...
  - RW
  - Combined with SCU2C[9] as bit[0]; 0–3.5 ns delay, optional clock inversion. [DS p.208]
* - 7
  - ARM CPU clock throttle enable
  - RW
  - 1=slow CPU for standby power saving (anti-glitch). [DS p.208]
* - 6:4
  - ARM CPU throttle ratio
  - RW
  - `000`=÷2 … `111`=÷16. [DS p.208]
* - 3:2
  - ECLK source select
  - RW
  - 00=M-PLL, 01=H-PLL, 10=inverted M-PLL, 11=inverted H-PLL. Stop ECLK + reset Video Engine before changing. [DS p.208]
* - 1:0
  - MCLK source select
  - RW
  - 00=M-PLL, 01=H-PLL, 10=inv M-PLL, 11=inv H-PLL. Change only at boot before DRAM init; disable MCLK (SCU0C[2]) first. [DS p.208]
```

### SCU0C — Clock Stop Control (offset 0x0C)

Per-clock gating. On most bits `1` = stop (gate off); several default to stopped
to save power until a driver enables the block. [DS p.209]
...
* - 19
  - Stop BHCLK (PCI host)
  - RW
  - 1=stop (default). [DS p.209]
* - 15
  - Stop UARTCLK (UART1/2)
  - RW
  - 0=run (default), 1=stop. [DS p.209]
* - 14
  - Enable USB2.0 clock
  - RW
  - 0=stop + power-down PHY (default); 1=run (then wait 10 ms before clearing SCU04[14]). [DS p.209]
* - 13
  - Stop YCLK (HAC)
  - RW
  - 1=stop (default). [DS p.209]
...
* - 3
  ...
  - RW
  - 1=stop (default). [DS p.210]
* - 2
  - Stop MCLK (SDRAM)
  - RW
  - 0=run (default). Disable before changing SCU08[1:0]. [DS p.210]
* - 1
  - Stop GCLK (2D engine)
  - RW
  - 1=stop (default). [DS p.210]
...
calibrate clocks against the 24 MHz reference. [DS p.210-211] [hwreg.h:83-84]
...
* - 7
  - Compare result
  - RO
  - 0=fail, 1=pass (SCU14 within SCU28 limits). [DS p.210]
* - 6
  - Measurement finished
  - RO
  - 0=not finished, 1=finished; cleared by SCU10[1]=0. [DS p.210]
* - 5:2
  - Measurement clock source
  - RW
  - 0011=D2-PLL, 0100=M-PLL, 0101=H-PLL, 0010=PCI, 0110=LPC, ring oscillators, video port clocks. [DS p.210]
* - 1
  - Oscillator counter enable
  - RW
  - 0=reset counter, 1=enable. [DS p.210]
* - 0
  - Enable ring oscillator
  - RW
  - 1=enable (wait 1 ms to settle). [DS p.210]
```
...
* - 15
  ...
  - RW
  - 1=cross-connect the two UART signal sets internally. [DS p.213]
* - 14
  - UART1 pin MUX enable
  - RW
  - 1=UART1 and UART2 share the UART1 pins/connector. [DS p.213]
* - 13
  - VUART timeout control
  - RW
  - See VUART20/VUART24. [DS p.213]
* - 12
  - UART div13 reference-clock enable
  - RW
  - 0: baud = 24MHz/(16·div); 1: baud = (24MHz/13)/(16·div). [DS p.213]
* - 11
  - Invert YCLK
  - RW
  - 0=YCLK from MCLK, 1=from inverted MCLK. [DS p.213]
* - 9
  - Video port-A delay bit[0]
  - RW
  - LSB of the SCU08[10:8] delay field. [DS p.213]
* - 8
  - Disable PCI-slave→AHB bridge
  - RW
  - 0=enable bridge, 1=disable. [DS p.213]
* - 2
  - Disable D1-PLL
  - RW
  - 0=enable, 1=disable. [DS p.213]
* - 0
...
Control (which is where the div13 bit lives). [hwreg.h:89] [platform.S:166-176] [DS p.213]
```

### SCU7C — Silicon Revision ID (offset 0x7C)

Read-only identity register. This is the register the DDR init reads to confirm
it is running on an AST2050/AST1100 before programming the M-PLL. [DS p.220]
...
* - 9:8
  - Chip bonding option
  - RO
  - Product-differentiation bonding status. [DS p.220]
* - 7:0
  - Silicon revision ID
  - RO
  - 0=A0, 1=A1, 2=A2/A3, … [DS p.220]
```
...
[DS §8.1 p.84, §8.5 p.88] [RAPTOR-PORTING-GUIDE.md:222-227]
...
[DS Figure 22 §8.5 p.88] [DS §8.1 p.84]
...
* - CPUCLK
  - H-PLL
  - 200 MHz
  - ARM926EJ-S core. [DS p.84]
* - HCLK
  - H-PLL
  - 100 MHz
  - AHB bus / most AHB masters+slaves. [DS p.84]
* - PCLK
  - H-PLL ÷ (SCU08[25:23])
  - 100 MHz
  - APB peripherals (timer, WDT, UART, I2C, LPC, GPIO, PWM, PECI). [DS p.84]
* - MCLK
  - M-PLL (or H-PLL)
  - 200 MHz
  - SDRAM controller. [DS p.84]
* - ECLK
  - M-PLL or H-PLL (SCU08[3:2])
  - 200 MHz
  - Video engine. [DS p.84]
* - YCLK / GCLK
  - MCLK
  - 200 MHz
  - HAC crypto / 2D engine. [DS p.84]
* - DCLK
  - V-PLL1
  - 165 MHz
  - CRT display. [DS p.84]
* - V1CLK
  - V1-PLL or DVI input
  - 165 MHz
  - Video capture #1. [DS p.84]
* - BCLK
  - external PCI clock
  - 33 MHz
  - PCI slave / bridge. [DS p.84]
* - LCLK
  - external LPC clock
  - 33 MHz
  - LPC controller. [DS p.84]
* - USB2CLK
  - USB2.0 PHY
  - 30 MHz
  - USB2.0 controller. [DS p.84]
* - UARTCLK
  - 24 MHz (opt. ÷13 via SCU2C[12])
  - 24 MHz
  - UART1/UART2 baud generation. [DS p.84, p.213]
* - CLK12M / CLK1M / CLK32K
  - 24 MHz ÷ integer
  - 12 / 1 / 0.032 MHz
  - RTC, timers, watchdog 1 MHz option. [DS p.84]
* - PECICLK / PWMCLK / TACHCLK
  - 24 MHz ÷ integer
  - 2 / 24 / 6 MHz
  - PECI, PWM/fan. [DS p.84]
```
...
[codebrowser clk-aspeed.c] [DS p.212]
...
* - 18 (SCU24 only)
  ...
  - RW
  - 0=take H-PLL parameters from trapping resistors; 1=from SCU24[17:0]. [DS p.212]
* - 17
  - Enable PLL bypass mode
  - RW
  - 1=output clock is the raw 24 MHz reference (shared with USB reference). [DS p.211-212]
* - 16
  - Turn off PLL
  - RW
  - 1=power down PLL, output forced to 0. M-PLL defaults OFF if SCU70[16] straps low-speed boot. [DS p.211-212]
* - 14:12
  - Post divider
  - RW
  - `0xx`=÷1, `100`=÷2, `101`=÷4, `110`=÷8, `111`=÷16. [DS p.211-212]
* - 10:5
  - Numerator (N)
  - RW
  - Feedback multiplier term (N+2). [DS p.211-212]
* - 4
  - Output divider (OD)
  - RW
  - Contributes `(2-OD)` to the formula. [DS p.211-212]
* - 3:0
  - Denumerator (D)
  - RW
  - Reference divider term (D+1). [DS p.211-212]
```
...
at its strap-selected value. [platform.S:158-159] [platform.S:338-340]
[platform.S:546-548]
...
until firmware sets SCU70[16]=1. [DS Figure 23 §8.5 p.88-89] [DS §SCU70 p.217-218]
...
200 MHz) when building a real G3 device tree. [RAPTOR_ENGINEERING_AST2050_ANALYSIS.md:1421-1433]
[DS §SCU24 p.212] [RAPTOR-PORTING-GUIDE.md:38]
```
...
[DS §SCU70 p.217-218] [pins ROMA*, DS p.42-43]
...
important porting hazard in this block. [DS §SCU70 p.217-218]
[RAPTOR-PORTING-GUIDE.md:41-42,229-232] [codebrowser clk-aspeed.c]
```
...
* - 31:24
  ...
  - RW
  - General-purpose strap scratch. [DS p.217]
* - 23
  - LPC dedicated reset pin
  - RW
  - 0=LPC reset shared with PCI reset pin; 1=LPC reset on pin B10. [DS p.217]
* - 22
  - Test mode
  - RW
  - 0=normal, 1=test mode (also forces bypass clock). [DS p.217]
* - 21
  - Reverse PCI AD[31:0] order
  - RW
  - PCB-routing option; swaps AD/CBE pin order. [DS p.217]
* - 20
  - Disable ARM→M-bus bridge
  - RW
  - 1=ARM reaches memory only via AHB. [DS p.217]
* - 19
  - Bypass all PLL
  - RW
  - 1=bypass every PLL (test only). [DS p.217]
* - 18
  - Reserved
  - RW
  - Keep 0. [DS p.217]
* - 17
  ...
  - RW
  - Value returned for the PCI config prefetch bit. [DS p.217]
* - 16
  - SOC boot-up full-speed
  - RW
  - 0=boot at 1/16 speed (throttled, M-PLL off, MCLK=24 MHz); 1=full speed. Firmware must set to 1 to leave low-speed mode. [DS p.217-218]
* - 15
  - PCI class-code select
  - RW
  - 0=video-device class code; 1=VGA-device class code. [DS p.218]
* - 14
  - Bypass VGA DAC
  - RW
  - Test-only DAC bypass. [DS p.218]
* - 13:12
  - **CPU:AHB clock ratio**
  - RW
  - 00=1:1, 01=2:1, 10=4:1, 11=3:1. (G4 uses [11:10].) [DS p.218]
* - 11:9
  - **H-PLL default frequency**
  - RW
  - `010`=200 MHz, `011`=166 MHz, `100`=133 MHz, `101`=100 MHz, `111`=24 MHz (H-PLL bypass); `00x`/`110`=reserved. (G4 uses [9:8].) [DS p.218]
* - 8:6
  - MAC interface mode
  - RW
  - `011`=MII (MAC#1) only, `100`=RMII (MAC#1) only, `110`=RMII MAC#1+MAC#2, `111`=disable MAC; others reserved. [DS p.218]
* - 5
  - Enable VGA BIOS ROM
  - RW
  - 0=no VGA BIOS ROM (on-board); 1=enable (add-on card). Also sets GPIOB4–B7 defaults. [DS p.218]
* - 4
  - Reserved
  - RW
  - Keep 0. [DS p.218]
* - 3:2
  - VGA memory size
  - RW
  - 00=8 MB, 01=16 MB, 10=32 MB, 11=64 MB, shared from SDRAM. [DS p.218]
* - 1:0
  - ARM CPU boot code select
  - RW
  - `10`=boot from SPI flash, `11`=disable ARM CPU, `0x`=reserved. [DS p.218]
```
...
this silicon. [platform.S:363-377]
...
`*_RST_N` = `PWRSTNin | SCU04[bit]`. [DS §8.6 p.94-96] [DS §SCU04 p.205-207]
[DS §SCU3C p.214]
...
- **`PWRSTNin`** (the `SRST#` power-on reset) asynchronously resets *all* SCU
  ...
  after power is stable. [DS §8.6 p.94] [pin R20 SRST#, DS p.47]
- **`HRST_N` = PWRSTNin | wdt_rst | (EXTRSTNin & SCU3C[3])**. So a watchdog
  ...
  into reset, but neither clears the SCU/RTC/power-domain state. [DS Figure 39 p.94]
- **`EXTRST#` (GPIOB7)** resets all SoC modules *except* the DRAM controller;
  it is enabled by `SCU3C[3]` and is active-low. [DS §SCU3C p.214] [pin C9, DS p.46]
- **`WDTRST` (GPIOB6, pin D9)** is the external watchdog-reset *output*, enabled
  by `SCU78[3]` + `WDT0C[3]`; it does not exist on A0 silicon. [DS p.46] [DS §SCU78 p.219] [DS §WDT0C p.288]
- The **RTC and GPIO** live in the `PWRSTNin` power domain (not `HRST_N`), so
  they survive watchdog/EXTRST resets. [DS §8.2 p.85]
```
...
"reserved, must keep at 1". [DS §SCU04 p.205-207]
...
* - 21
  ...
  - RW
  - Controls BRST# pin direction; 0=input (default). [DS p.205]
* - 20
  - Force PCI host reset high
  - RW
  - 1=hold BRST# high while resetting host controller. [DS p.205]
* - 19
  - Reset PCI host bus controller
  - RW
  - 1=reset (default). [DS p.205]
* - 18
...
* - 14
  ...
  - RW
  - 1=reset (default). Clear only after enabling USB2.0 clock + 10 ms wait. [DS p.205]
* - 13
  - Reserved (keep 1)
  - RW
  - Must stay 1. [DS p.205]
...
* - 8
  - Reset PCI slave + VGA
  - RW
  - 1=reset; 0=no operation (default). [DS p.206]
* - 7
  - Reserved (keep 0)
  - RW
  - Must stay 0. [DS p.206]
* - 6
  - Reset video engine
  - RW
  - 1=reset (default). [DS p.206]
* - 5
  ...
  - RW
  - 1=reset LPC + embedded BMC controller; 0=default. [DS p.206]
* - 4
  - Reset HAC engine
  - RW
  - 1=reset (default). [DS p.206]
* - 3
  - Reserved (keep 1)
  - RW
  - Must stay 1. [DS p.206]
* - 2
  - Reset I2C/SMBus
  - RW
  - 1=reset all 7 controllers (default). [DS p.206]
* - 1
  - Reset AHB bridges
  - RW
  - 1=reset AHB↔M-bus, AHB↔APB, AHB↔P-bus bridges; write 0 to resume. [DS p.206]
* - 0
  - Reset SDRAM controller
  - RW
  - 1=reset (may lose DRAM contents); 0=default. [DS p.207]
```
...
* - 3
  ...
  - RW
  - 1=GPIOB7 acts as active-low EXTRST# (resets SoC except DRAM). Cleared by SRST#; software may set after boot. Keep pin high when enabling. [DS p.214]
* - 2
  - External reset flag
  - R/WSC
  - Cleared by SRST#; **set when EXTRST# fired**. Software clears after reading. [DS p.214]
* - 1
  - Watchdog reset flag
  - R/WSC
  - Cleared by SRST#; **set when the internal watchdog reset fired**. Software clears after reading. [DS p.214]
* - 0
  - Power-on reset flag
  - R/WSC
  - **Set by SRST#** (power-on). Software clears after reading. [DS p.214]
```
...
PCLK or the 1 MHz reference (selected by `WDT0C[4]`). [DS §27 p.287-289]
[hwreg.h:164-169] [DS §8.2 p.85]
...
* - 0x00
  ...
  - 0x03EF1480
  - RO
  - Current counter value; resets to `0x03EF1480` on `HRST_N`; loaded from WDT04 on restart. Counts down while WDT0C[0]=1. [DS p.287] [hwreg.h:166]
* - 0x04
  - WDT04 Counter Reload Value
  - 0x03EF1480
  - RW
  - Value loaded into WDT00 on reset/restart; sets the timeout period. [DS p.288] [hwreg.h:167]
* - 0x08
  - WDT08 Counter Restart
  - 0x00000000
  - WO
  - Write `0x4755` (bits[15:0]) to reload WDT00 and (re)start counting — the "kick". [DS p.288] [hwreg.h:168]
* - 0x0C
  - WDT0C Control
  - 0x00000000
  - RW
  - Enable, reset-on-timeout, interrupt-on-timeout, external-signal-on-timeout, clock select. [DS p.288] [hwreg.h:169]
* - 0x10
  - WDT10 Timeout Status
  - 0x00000000
  - RO
  - Bit0=1 latches that a timeout has ever occurred. [DS p.288]
* - 0x14
  - WDT14 Clear Timeout Status
  - 0x00000000
  - W1C
  - Write 1 to bit0 to clear WDT10. [DS p.288]
* - 0x18
  - WDT18 Reset Width
  - 0x000000FF
  - RW
  - Asserting width of wdt_intr / wdt_ext in PCLK cycles (default 256). With 1 MHz clock, max ~1.25 µs. [DS p.288-289]
```
...
* - 4
  - Clock select
  - RW
  - 0=PCLK, 1=1 MHz source. [DS p.288]
* - 3
  - External signal enable (wdt_ext)
  - RW
  - 1=drive external `WDTRST` output pin D9 high on timeout. [DS p.288]
* - 2
  - Interrupt enable (wdt_intr)
  - RW
  - 1=raise CPU interrupt on timeout. [DS p.288]
* - 1
  - Reset system on timeout
  - RW
  - 1=assert system reset on timeout (feeds `HRST_N`). [DS p.288]
* - 0
  - WDT enable
  - RW
  - 1=counter runs. [DS p.288]
```
...
domains above). [DS §27.4 p.289] [DS Figure 39 §8.6 p.94]
...
but belong to this subsystem's programming model. [DS §17 p.200-201]
[hwreg.h:70-71]
...
* - 0x100
  ...
  - 0x000000A8
  - RO
  - Fixed `0x000000A8`. [DS p.201] [hwreg.h:70]
* - 0x120
  - MCR120 AST2000-compat SCU M-PLL parameter
  - 0x00000000
  - RW
  - Post-divider [15:14], numerator [13:5], denumerator [4:0]. DDR init writes `0x00004c41`. [DS p.201] [platform.S:546-548]
* - 0x170
  - MCR170 AST2000-compat SCU hardware-strap value
  - 0x00000000
  - RO
  - Reads all 0. [DS p.201]
```
...
1. **Unlock SCU** — write `0x1688A8A8` to `SCU00`. [platform.S:132-134,240-242]
2. **Claim DRAM init** — set `SCU40[7]` (SOC-firmware-initialises-DRAM). [platform.S:136-139]
3. **Skip if already done** — read `SCU40[6]`; if the DRAM-init-ready flag is
   already set, jump straight to the lock/return path. [platform.S:141-147]
4. **Detect silicon** — read `SCU7C`, shift right 8, compare low byte against
   `0x02` to confirm AST2050/AST1100 before touching the PLL. [platform.S:150-154]
...
6. **Pick UART baud** — read the misc/div13 control (SCU2C[12]) to choose the
   baud divisor, then bring up UART2 for debug output. [platform.S:162-225]
7. **Verify unlock** — re-write `SCU00 = 0x1688A8A8` and read back, requiring
   `0x01`; abort with "SCU LOCKED" if not. [platform.S:240-247]
8. **Set boot key** — `SCU40 = 0x5A000080` (Linux-boot key `0x5A` in [31:24] +
   DRAM-init-by-firmware bit[7]). [platform.S:283-285]
9. **Unlock SDRAM** — write `0xFC600309` to the SDRAM protection key (`MCR00`),
   verify. [platform.S:287-295]
10. **Program M-PLL (second pass)** — `SCU20 = 0x000041f0` (numerator 0b001111,
    OD=1, post-÷2), then the full DDR2 timing/DLL init. [platform.S:338-340,354-544]
11. **Read VGA-size strap** — `SCU70[3:2]` folded into `MCR04` SDRAM config.
    [platform.S:363-377]
12. **Write MPLL shadow** — `MCR120 = 0x00004c41` (AST2000-compat). [platform.S:546-548]
13. **Signal done** — set `SCU40[6]` (DRAM-init-ready). [platform.S:559-564]
...
sequence. The `0x033103F1` value loaded into `r2` at [platform.S:149] before the
silicon check is dead/unused (the subsequent `set_MPLL` writes literal
`0x00004c81` instead). [platform.S:149-159]
