...
static memory is *not* supported [HWRef p.196-197].
...
Offsets confirmed by [mach-ns9xxx regs-mem.h] (`MEM_CTRL`=0xa0700000,
`MEM_DMCONF`=0x100+8x, `MEM_SMC`=0x200+8x) and [u-boot ns9750_mem.h].

### Control, Status, Config

```{list-table} Control register (0x000) [HWRef p.284-285]
:header-rows: 1
:widths: 12 14 12 62
...
status, bit 0 `BUSY` [HWRef p.286]. `Config` (0x008): bit 0 `END` endian
...
```{list-table} DynamicControl register (0x020) [HWRef p.287-289]
:header-rows: 1
:widths: 12 14 12 62
...
HCLK ticks between refreshes [HWRef p.289]. `DynamicReadConfig` (0x028) bits 1:0
`RD`: command-delayed capture strategy (01/10/11 = +0/+1/+2 clock) [HWRef p.291].

The SDRAM timing registers each hold one field (value n encodes n+1 clocks):

```{list-table} Dynamic timing registers (value = n+1 clocks) [HWRef p.292-302]
:header-rows: 1
:widths: 20 14 12 54
...
```{list-table} DynamicConfig register (0x100 + n*0x20) [HWRef p.304-307]
:header-rows: 1
:widths: 12 14 12 62
...
= 001, bits 8:7 = 01 [HWRef p.305-307].
...
[HWRef p.308]. For the board's IS42S32800D at the operating AHB clock the
port draft uses CAS = 2, RAS = 3 [PLAN-INCREMENTAL-PORT.md].

### Static (flash / SRAM) controller

```{list-table} StaticConfig register (0x200 + n*0x20) [HWRef p.309-312]
:header-rows: 1
:widths: 12 14 12 62
...
Field bits match [mach-ns9xxx regs-mem.h] (`MEM_SMC_EW`=8, `MEM_SMC_PB`=7,
`MEM_SMC_PC`=6, `MEM_SMC_PM`=3, `MEM_SMC_MW`=1:0) and [u-boot ns9750_mem.h].
...
[HWRef p.313-318]. The shared `StaticExtendedWait` (0x080) times CS whose `EW`
bit is set: bits 9:0 encode (n+1)×16 HCLK cycles [HWRef p.303]. On this board the
...
receive descriptor rings (A/B/C/D) plus one transmit ring [HWRef p.328-334].
...
Offsets and mnemonics confirmed by [u-boot ns9750_eth.h] (`EGCR1`=0x000,
`MAC1`=0x400, `MCFG`=0x420, `SA1`=0x440, `RXAPTR`=0xA00, `TXBD`=0x1000, etc.).

### General control / status

```{list-table} EGCR1 — General Control 1 (0x000) [HWRef p.343-346]
:header-rows: 1
:widths: 12 14 12 62
...
(clear statistics), bit 0 `STEN` (enable statistics) [HWRef p.346-347]. `EGSR`
(0x008): bit 20 `RXINIT` = RX descriptor init complete [HWRef p.348]. `ETSR`
...
copied into the closing buffer descriptor [HWRef p.348-353].

### MAC configuration

```{list-table} MAC1 (0x400) [HWRef p.354-355]
:header-rows: 1
:widths: 12 14 12 62
...
```{list-table} MAC2 (0x404) — key fields [HWRef p.355-357]
:header-rows: 1
:widths: 12 14 12 62
...
(0x414, max frame length, reset 0x0600 = 1536) [HWRef p.358-361]. `SUPP` (0x418):
bit 15 `RPERMII` (reset RMII), bit 8 `SPEED` (RMII 10/100) [HWRef p.362]. MAC1/MAC2
field bits match [u-boot ns9750_eth.h] (`MAC1_SRST`=0x8000, `MAC1_RXEN`=0x0001,
`MAC2_FULLD`=0x0001, `MAC2_CRCEN`=0x0010, `MAC2_PADEN`=0x0020).

### MII management (PHY access)

```{list-table} MII management registers [HWRef p.363-369]
:header-rows: 1
:widths: 14 12 74
...
divide the AHB clock accordingly (e.g. ÷40) [HWRef p.363-366]. The board's PHY is
at MDIO address 1 [REFERENCE-MATERIAL.md].

### Station address, filtering, descriptors

`SA1`/`SA2`/`SA3` (0x440/0x444/0x448) hold the 48-bit station address, two octets
per register; note octet 6 is the first byte on the wire [HWRef p.369-371].
`SAFR` (0x500): bit 3 `PRO` (promiscuous), bit 2 `PRM` (all multicast), bit 1
`PRA` (hash-filtered multicast), bit 0 `BROAD` (broadcast) [HWRef p.371]. `HT1`/
`HT2` (0x504/0x508) are the 64-bit multicast hash table indexed by the top 6 bits
of the destination-address CRC [HWRef p.372-373].
...
[HWRef p.329-332, p.403]. `EINTR` (0xA10) reports RX events (bits 25:16:
...
mask [HWRef p.391-395]. `RXFREE` (0xA3C, write-only, per-pool) tells the receiver
a buffer was freed; `TXERBD` (0xA20) points at a failed TX descriptor for recovery
via `TXRPTR` + `EGCR2.TCLER` [HWRef p.396-402].
...
undersize/fragment) [HWRef p.374-388]. Enable via `EGCR2.STEN`; carry bits set
...
channel C = 0x9030_0000, channel D = 0x9030_0040 [HWRef p.571-572]
[u-boot ns9750_ser.h]. Each channel is a 0x40-byte block with identical layout,
...
Offsets confirmed by [u-boot ns9750_ser.h] (`CTRL_A`=0x00, `CTRL_B`=0x04,
...
```{list-table} UART Control Register A [HWRef p.574-577]
:header-rows: 1
:widths: 12 14 12 62
...
```{list-table} UART Status Register A — selected bits [HWRef p.580-586]
:header-rows: 1
:widths: 12 14 12 62
...
```{list-table} Bit-rate register [HWRef p.587-591]
:header-rows: 1
:widths: 12 14 12 62
...
Field bits match [u-boot ns9750_ser.h] (`EBIT`=0x80000000, `TMODE`=0x40000000,
...
selects BCLK = the BBus clock = AHB/2 = f_vco/8 ≈ 44.24 MHz [HWRef p.566 Table 400].
Example N values for x1_sys_osc/2 at 16× oversampling: 115200 baud → N = 7,
9600 baud → N = 95, 38400 baud → N = 23 [HWRef p.591, Table 407].
...
[u-boot ns9750_serial.c]. With the correct PLL/system clock (≈354 MHz), that
...
RX buffers on idle/pattern and implement XON/XOFF [HWRef p.592-600].
...
are unused [HWRef p.601-609]. In SPI: word length is forced to 8 bits;
...
by the `SPCPOL`+`TXCINV`+`RXCINV` combination [HWRef p.605, p.610-623]. SPI master
rate = BCLK/4 (max 11.07 Mbps); slave clock comes from the external master. The
recommended SPI clock source is BCLK (`CLKMUX` = 01) [HWRef p.603, p.608]. The
...
**Base address: 0x9050_0000** [HWRef p.512]. A combined master/slave controller
...
(400 kHz) timing [HWRef p.507-509]. The board wires SDA/SCL to gpio[35]/gpio[34]
[ANALYSIS.md]. The firmware uses this bus actively [ANALYSIS.md].

```{list-table} I2C registers (offset from 0x9050_0000) [HWRef p.512]
:header-rows: 1
:widths: 14 30 56
...
```{list-table} Command Transmit Data (write, 0x00) [HWRef p.513]
:header-rows: 1
:widths: 12 14 12 62
...
```{list-table} Status Receive Data (read, 0x00) [HWRef p.514]
:header-rows: 1
:widths: 12 14 12 62
...
0x06 `M_STOP`; slave commands 0x10 `S_NOP`, 0x16 `S_STOP` [HWRef p.510-511].
`Master Address` (0x04): bits 10:1 device address, bit 0 `MAM` (0 = 7-bit,
1 = 10-bit) [HWRef p.515]. `Slave Address` (0x08): bit 11 `GCA` (general-call
enable, default 1), bits 10:1 `SDA` (default 0x7F), bit 0 `SAM` [HWRef p.516].
...
+ 4 + scl_delay), clk = cpu_clk/4) [HWRef p.517-518]. Interrupt codes reported in
`IRQCD` include `M_ARBIT_LOST` (1), `M_NO_ACK` (2), `M_TX_DATA` (3), `M_RX_DATA`
(4), `M_CMD_ACK` (5), and slave codes 8-F [HWRef p.518-519].

## Real Time Clock


**Base address: 0x9070_0000** [HWRef p.491]. Tracks time-of-day to 10 ms with a
full calendar (year 1900-2999), an alarm, and rollover-event interrupts
[HWRef p.489-490]. Time and calendar values are BCD. The board adds a coin-cell
backup [ANALYSIS.md].

```{list-table} RTC registers (offset from 0x9070_0000) [HWRef p.491]
:header-rows: 1
:widths: 14 26 60
...
1 = disabled (both reset disabled) [HWRef p.492]. `Alarm Enable` (0x18) bits 5:0
...
will not run on an invalid configuration [HWRef p.498-506]. The RTC clock divider
that produces the 100 Hz tick lives in the SCM at 0xA0900224 [HWRef p.193].
