...
## LCD Controller (secondary)


**Base address: 0xA0800000** [HWRef p.543]. An AHB-master LCD controller for TFT
(18-bit) and STN (mono/colour, single/dual panel) panels up to 1024×768 and up to
64K colours; a 256-entry palette RAM sits at 0xA0800200-0xA08003FC and is used by
boot/endian-switch code as scratch RAM [HWRef p.524-543][PLAN-INCREMENTAL-PORT.md].
The board does not use the LCD block, but it must be listed because the endian
switch executes from its palette RAM.

```{list-table} LCD controller registers (offset from 0xA0800000) [HWRef p.543]
:header-rows: 1
:widths: 16 24 60
...
* - 0x000-0x00C
  - LCDTiming0-3
  - Horizontal / vertical / clock-polarity / line-end timing
* - 0x010 / 0x014
  - LCDUPBASE / LCDLPBASE
  - Upper / lower panel DMA frame base
* - 0x018
  - LCDINTRENABLE
  - Interrupt enable mask
* - 0x01C
  - LCDControl
  - Enable, bpp, TFT/STN, mono, dual, power, colour order
* - 0x020 / 0x024
  - LCDStatus / LCDInterrupt
  - Raw / masked interrupt status
* - 0x028 / 0x02C
  - LCDUPCURR / LCDLPCURR
  - Current DMA addresses (read-only)
* - 0x200-0x3FC
  - LCDPalette
  - 256-entry × 16-bit palette (also boot scratch RAM)
```

Two interrupts: `MBERRORINTR` (AHB master bus error) and `VCOMPINTR` (vertical
compare), plus a next-base-update interrupt [HWRef p.561-562].

## IEEE 1284 Parallel Port (secondary)


**Base address: 0x9040_0000** [HWRef p.635]. An IEEE-1284 peripheral-side parallel
port (compatibility/nibble/byte/ECP modes; no EPP) with DMA or direct-FIFO modes
[HWRef p.626-637]. Unused on this board.

```{list-table} IEEE 1284 registers — top level (offset from 0x9040_0000) [HWRef p.635-637]
:header-rows: 1
:widths: 16 30 54
...
* - 0x0000
  - GenConfig
  - General configuration (thresholds, DMA-vs-CPU modes)
* - 0x0004
  - Interrupt Status and Control
  - FIFO/peripheral interrupt status + mask
* - 0x0008
  - FIFO Status
  - Forward/reverse FIFO levels
* - 0x000C / 0x0010
  - Forward Command / Data FIFO Read
  - CPU-mode FIFO reads
* - 0x001C / 0x0020
  - Reverse FIFO Write / Write-Last
  - CPU-mode reverse writes
* - 0x0024 / 0x0028
  - Forward Command / Data DMA Control
  - Max buffer size + byte-gap timer
* - 0x0100-0x017C
  - CSR block
  - Printer-data, port status/control, feature/master enable, phase
```

## USB Host (secondary)


**Base address: 0x9080_0000** [HWRef p.667]. An OHCI-compliant USB 2.0 host
(full/low speed) with a USB Host Front End (UHFE) bridging OHCI to the BBus. Sideband
signals: USB_PWR = gpio[17], USB_OVR = gpio[16] [HWRef p.663-672]. Unused in the
stock firmware.

```{list-table} USB host address map [HWRef p.667-672]
:header-rows: 1
:widths: 30 70
...
* - 0x9080_0000-0x9080_0FFF
  - UHFE control/status (Control+Status 0x0000, Interrupt Enable 0x000C, Interrupt Status 0x0010)
* - 0x9080_1000-0x9080_1FFF
  - Standard OHCI registers (HcRevision 0x000, HcControl 0x004, HcCommandStatus 0x008, HcInterruptStatus 0x00C, … HcRhPortStatus 0x054)
```

The OHCI register block follows the OpenHCI specification exactly, based at
0x9080_1000 [HWRef p.671-672].

## USB Device (secondary)


**Base address: 0x9090_0000** [HWRef p.723]. A USB 2.0 device with 11 endpoints
(one bidirectional control endpoint 0 + up to 10 unidirectional), a 13-channel
BBus DMA controller (one per endpoint), and a device front end (UDFE)
[HWRef p.711-723]. Unused in the stock firmware.

```{list-table} USB device address map [HWRef p.723-724]
:header-rows: 1
:widths: 30 70
...
* - 0x9090_0000-0x9090_0FFF
  - UDFE control/status (Control/Status 0 = 0x0000, Control/Status 1 = 0x0004, Interrupt Enable 0x000C, Interrupt Status 0x0010, Programming Control/Status 0x0014)
* - 0x9090_2000-0x9090_2FFF
  - USB device controller block (endpoint descriptors)
* - 0x9090_3000-0x9090_3FFF
  - Endpoint FIFOs
* - 0x9091_0000-0x9091_FFFF
  - USB device DMA (BBus DMA controller 2)
```
