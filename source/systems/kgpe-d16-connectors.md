...
```{figure} /_static/images/kgpe-d16-manual-bmc-fw1.png
:alt: ASUS manual excerpt — "BMC header (BMC_FW1)": board-location drawing with the 2x7 header circled at the board's lower-left, a Pin1 marker, and the sentence "The BMC connector on the motherboard supports an ASUS Server Management Board 4 Series (ASMB4)".
:width: 100%

The header as the **ASUS KGPE-D16 User Guide** presents it ("BMC header
(BMC_FW1)"): board location and pin 1 only — the manual page that seeded the
old "ASMB4 slot" identity. Both readings are consistent: the ASMB4 module is
a firmware daughtercard that seats in this socket (the manual's installation
photos are on {doc}`kgpe-d16` §2.3). © ASUSTeK Computer Inc.; the full manual
is
...
```{figure} /_static/images/kgpe-d16-manual-tpm1.png
:alt: ASUS manual excerpt — "TPM connector (20-1 pin TPM)": board-location drawing with the header circled near the board's lower-right, and a pinout naming PCICLK, FRAME, PCIRST#, LAD0–LAD3, SERIRQ, SMBSCL/SMBSDA, +3V, +3VSB, GND, PWROWN, GPZO, RESET and SB_SUS_STAT.
:width: 100%

The same header in the **ASUS KGPE-D16 User Guide** ("TPM connector (20-1 pin
TPM)"). ASUS's pin names corroborate the netlist reading above — `PCICLK` is
the TPM's own LPC clock, `PCIRST#` the Super-I/O reset, and `LAD0–3` /
`FRAME` / `SERIRQ` are the lines shared with the BMC — and additionally name
the pins the netlist only resolves as pull networks: `SMBSCL`/`SMBSDA`,
`+3VSB`, `PWROWN`, `GPZO`, `RESET` and `SB_SUS_STAT`. © ASUSTeK Computer
Inc.; the full manual is
[mirrored in the program repo](https://github.com/mithro/ai-shenanigans-for-bmcs/blob/main/asus-kgpe-d16-firmware/datasheets/KGPE-D16_Manual.pdf).
```
...
- **[ASUS KGPE-D16 User Guide](https://github.com/mithro/ai-shenanigans-for-bmcs/blob/main/asus-kgpe-d16-firmware/datasheets/KGPE-D16_Manual.pdf)** — the vendor manual; the `TPM1`
  and `BMC_FW1` figures above are reproduced from it (© ASUSTeK).
